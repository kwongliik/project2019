...
class InventoriEditViewTestCase(TestCase):
    ...
    '''
    def setUp(self):
        self.pembekal = Pembekal.objects.create(pembekal='Peter S/B', alamat='sibu', telefon='112233')
        self.username = 'john'
        self.password = '123'
        user = User.objects.create_user(username=self.username, email='john@doe.com', password=self.password)
        self.stok = Stok.objects.create(stok='buku', pembekal=self.pembekal)
        self.inventori = Inventori.objects.create(inventori='buku latihan', stok=self.stok, harga=2.00, kuantiti=100, created_by=user)
...
class InventoriEditViewTests(InventoriEditViewTestCase):
    ...
    def test_form_inputs(self):
        '''
        The view must contain four inputs: csrf, inventori, harga and kuantiti
        '''
...
class SuccessfulInventoriEditViewTests(InventoriEditViewTestCase):
    def setUp(self):
        super().setUp()
        self.client.login(username=self.username, password=self.password)
        self.response = self.client.post(self.url, {'inventori': 'buku latihan', 'stok': self.stok, 'harga': '3.00', 'kuantiti': '200'})
    ...
    def test_inventori_changed(self):
        ...
        self.assertEquals(self.inventori.inventori, 'buku latihan')
