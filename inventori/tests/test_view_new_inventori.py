...
class NewInventoriTestCase(TestCase):
    '''
    Base test case to be used in all `new inventori` view tests
    '''
    def setUp(self):
        self.pembekal = Pembekal.objects.create(pembekal='Cemerlang s/b', alamat='sibu', telefon='223344')
        self.username = 'john'
        self.password = '123'
        user = User.objects.create_user(username=self.username, email='john@doe.com', password=self.password)
        self.stok = Stok.objects.create(stok='alatulis', pembekal=self.pembekal)
        Inventori.objects.create(inventori='pencil', stok=self.stok, harga=1.50, kuantiti=100, created_by=user)
...
class NewInventoriTests(NewInventoriTestCase):
    ...
    def test_form_inputs(self):
        '''
        The view must contain four inputs: csrf, inventori, harga, kuantiti
        '''
...
class SuccessfulNewInventoriTests(NewInventoriTestCase):
    def setUp(self):
        super().setUp()
        self.client.login(username=self.username, password=self.password)
        self.response = self.client.post(self.url, {'inventori': 'pen', 'harga':2.00, 'kuantiti':100})
...
class InvalidNewInventoriTests(NewInventoriTestCase):
    def setUp(self):
        '''
        Submit an empty dictionary to the `new inventori` view
        '''
        super().setUp()
        self.client.login(username=self.username, password=self.password)
